...
export default function ProfileEditor({ profile, onChange, onClose }: Props) {
  ...
  return (
    <div className="no-print space-y-4 rounded-xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-900">Search profile</h3>
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-wide text-slate-400">Commute anchors</span>
          <button onClick={addAnchor} className="rounded-lg border border-slate-300 px-2 py-1 text-xs font-medium hover:border-slate-400">
            + Add anchor
          </button>
        </div>
        <div className="space-y-2">
          {profile.anchors.length === 0 && (
            <p className="text-xs text-slate-400">No anchors yet. Add places you commute to (office, downtown…) to score & rank by commute.</p>
          )}
          {profile.anchors.map((a) => (
            <div key={a.id} className="grid items-end gap-2 rounded-lg border border-slate-200 p-2 sm:grid-cols-2 lg:grid-cols-6">
              <Field label="Label">
      ...
      {/* Amenities */}
      <div>
        <div className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-400">Amenity weights</div>
